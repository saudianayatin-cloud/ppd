...
if (isset($_POST['stud_id8'])) {
    $stud_id = mysqli_real_escape_string($conn, $_POST['stud_id8']);

    // Fetch file names first
    $query = mysqli_query($conn, "SELECT file, file2, file3 FROM `documents` WHERE `stud_id8` = '$stud_id'");
    ...
    if ($query && mysqli_num_rows($query) > 0) {
        ...
        foreach (['file', 'file2', 'file3'] as $col) {
            if (!empty($fetch[$col])) {
        ...
        if (mysqli_query($conn, "DELETE FROM `documents` WHERE `stud_id8` = '$stud_id'")) {
