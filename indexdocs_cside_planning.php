...
<body>
    ...
    <div class="container">
        ...
        <h3 style="text-align: left; color: #0a376e;">Document List(Planning)</h3>
    ...
    <div class="modal fade" id="form_modal" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form id="add_document_form" method="POST" action="save_documents_cside_planning.php" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>Code<a style="color: red; font-size:large">*</a></label>

                            <?php
                            $randnumber = rand(1000000, 9999999); // A larger range for random number
                            // $uniqueID = uniqid(); // Generates a unique ID based on current time in microseconds
                            $currentDate = date('Ymd'); // Format the date as YYYYMMDD
                            // $generatedValue = $randnumber . '-' . $uniqueID . '-' . $currentDate; // Combine them all
                            $generatedValue = $currentDate . '-' . $randnumber; // Combine them all
                            ?>

                            <input style="text-align:center" name="stud_no" class="form-control" type="text" value="<?= $generatedValue ?>" required readonly>


                        </div>
                        <div class="form-group">
                            <label>Subjects<a style="color: red; font-size:large">*</a><br></label>
                            <textarea class="form-control" name="subject" rows="4" required="required"></textarea>
                        </div>
                        <div class="form-group">
                            <label class="required">Date Received:<a style="color: red; font-size:large;">*</a></label>
                            <input type="date" name="date" class="form-control" required="required" />
                        </div>

                        <div class="form-group">
                            <label>Concerned Agencies:<a style="color: red; font-size:large;">*</a></label>
                            <textarea class="form-control" name="con_agency" rows="1" required="required"></textarea>
                        </div>
                        <div class="form-group">
                            <label>Forwarded to:<a style="color: red; font-size:large;">*</a></label>
                            <textarea class="form-control" name="forwarded" rows="1" required="required"></textarea>
                        </div>


                        <div class="form-group">
                            <label>File Status<a style="color: red; font-size:large">*</a></label>
                            <select name="status" class="form-control" required="required">
                                <option value="Uploaded">Uploaded</option>
                                <option value="Not yet Uploaded">Not yet Uploaded</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            <label>Attach/Insert(URL link)<a style="color: red; font-size:large">*</a></label>
                            <textarea placeholder="here..." class="form-control" name="attachment" rows="4"></textarea>
                        </div>
    ...
    <div class="modal fade" id="edit_modal" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form method="POST" action="update_documents_cside_planning.php" enctype="multipart/form-data">
                    <div class="modal-header">
                        <h4 class="modal-title"><span class="glyphicon glyphicon-edit"></span> Update</h4>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
                        <input type="hidden" name="stud_id4" id="stud_id4" />

                        <div class="form-group">
                            <label>Code</label>
                            <input style="text-align: center;" class="form-control" name="stud_no" id="stud_no">
                        </div>
                        ...
                        <div class="form-group">
                            <label>Concerned Agencies</label>
                            <textarea class="form-control" name="con_agency" id="con_agency" rows="1"></textarea>
                        ...
                        <div class="form-group">
                            <label>Attach/Insert(URL link)</label>
                            <textarea class="form-control" name="attachment" id="attachment" rows="4"></textarea>
                        </div>
                        <div class="form-group">
                            <label>Forwarded to</label>
                            <textarea class="form-control" name="forwarded" id="forwarded" rows="1"></textarea>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editForm.addEventListener('submit', function(e) {
                ...
                const editedId = document.getElementById('stud_id4').value;
                ...
                xhr.addEventListener('load', function() {
                    ...
                    if ($.fn.DataTable.isDataTable('#your-table')) {
                        ...
                        $.ajax({
                            url: 'get_single_document_planning.php',
                            method: 'GET',
                            data: {
                                id: editedId
                            },
                            ...
                            success: function(newData) {
                                if (newData && newData.stud_id4) {
                                    const rowSelector = '#' + newData.stud_id4;
    ...
    <script>
        function openModal(rowData) {
            document.getElementById('stud_id4').value = rowData.stud_id4 || '';
            ...
            document.getElementById('con_agency').value = rowData.con_agency || '';
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn-delete', function() {
                var stud_id4 = $(this).data('id');
                console.log("Deleting ID:", stud_id4);
                $("#modal_confirm").modal('show');
                $('#btn_yes').attr('data-id', stud_id4);
            });
            ...
            $('#btn_yes').on('click', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "delete_documents_planning.php",
                    data: {
                        stud_id4: id
                    },
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn-delete', function() {
                var stud_id4 = $(this).data('id'); // ✅ use data-id instead of id
                console.log("Deleting ID:", stud_id4);
                $("#modal_confirm").modal('show');
                $('#btn_yes').attr('name', stud_id4);
            });
